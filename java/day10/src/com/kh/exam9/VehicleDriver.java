...
public class VehicleDriver {
    public static void main(String[] args) {
        ...
        System.out.println(car1.calculatedTax());

        // 2,500 톤의 가격이 2,000 만원인 봉고 트럭을 나타내는 Truck 객체를 생성한다
        Truck truck = new Truck("봉고 트럭", 20_000_000, 2_500);

        // truck이 가리키는 객체의 모든 데이터를 출력한다
        System.out.println(truck.toString());

        // truck이 가리키는 객체의 세금을 계산하여 출력한다
        System.out.println(truck.calculatedTax());
        ...
        System.out.println(car2.calculatedTax());
        ...
        for (int i = 0; i < vehicles.length; i++) {
            System.out.println(vehicles[i].calculatedTax());
        }
    }
}